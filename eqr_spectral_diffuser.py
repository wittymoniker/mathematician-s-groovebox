# Filename: eqr_spectral_diffuser.py
...
class SpectralDiffuserPage(QWidget):
    """
    EQR Spectral Diffuser & Harmonic Dispersion Module built with
    skeuomorphic rotary patch controls and all 34 geometric constants.
    """
    def __init__(self, engine):
        ...
        layout = QVBoxLayout()
        group = QGroupBox("EQR Spectral Diffuser & Harmonic Dispersion Matrix")
        grid = QGridLayout()

        diffuser_channels = [
            ("Diffuser Spread Depth", 0.0, 100.0, 43.2),
            ("Frequency Smear Rate", 0.1, 20.0, 2.5),
            ("Apéry Dispersion Factor", 1.0, 2.0, EQR_CONSTANTS["APERY_CONSTANT"]),
            ("Lemniscate Resonance", 1.0, 4.0, EQR_CONSTANTS["GAUSS_LEMNISCATE"]),
            ("Khinchin Fraction Limit", 1.0, 5.0, EQR_CONSTANTS["KHINCHIN_CONST"]),
            ("Glaisher Scaling Matrix", 1.0, 2.0, EQR_CONSTANTS["GLAISHER_KINKEL"]),
            ("Twin Prime Harmonic Shift", 0.1, 2.0, EQR_CONSTANTS["TWIN_PRIME"]),
            ("MEUM Diffuser Lock", 0.0, 2.0, MEUM)
        ]

        for idx, (label, min_v, max_v, def_v) in enumerate(diffuser_channels):
            r = idx // 4
            c = idx % 4
