...
# Credits / maintenance assistance: Grok (xAI), Gemini (Google), ChatGPT (OpenAI).
...
class VideoSynthEngine:
    """Lightweight waveform-driven 2.5D visual field used by the live audiovisual pane."""
    def __init__(self, n_instruments=48):
        self.wave=np.zeros(256,dtype=np.float32); self.t=0.0; self.n=n_instruments
    def set_waveform(self,data):
        if data is None: return
        a=np.asarray(data,dtype=np.float32).ravel()
        if a.size: self.wave=np.interp(np.linspace(0,a.size-1,256),np.arange(a.size),a).astype(np.float32); self.t+=0.04
    def render_frame(self,w,h):
        img=np.zeros((h,w,3),dtype=np.uint8); e=float(np.sqrt(np.mean(self.wave*self.wave))+1e-6)
        img[:]=np.array([int(5+18*e),int(8+24*e),int(12+30*e)],dtype=np.uint8)
        for i in range(255):
            x0=int(i/255*(w-1)); x1=int((i+1)/255*(w-1)); y0=int(h*.72-self.wave[i]*h*.18); y1=int(h*.72-self.wave[i+1]*h*.18)
            steps=max(abs(x1-x0),abs(y1-y0),1)
            for k in range(steps+1):
                u=k/steps; x=int(x0+(x1-x0)*u); y=int(y0+(y1-y0)*u)
                if 0<=x<w and 0<=y<h: img[y,x]=[0,210,170]
        for k in range(16):
            a=self.t*.35+k*.392; r=(.12+.5*abs(self.wave[(k*17)%256]))*min(w,h); cx=w*.5+r*np.cos(a); cy=h*.48+r*.42*np.sin(a)
            rr=max(2,int(3+5*abs(self.wave[(k*17)%256])))
            yy,xx=np.ogrid[:h,:w]; mask=(xx-cx)**2+(yy-cy)**2<=rr*rr; img[mask]=[180,90+(k*5)%120,255]
        return img

class VideoSynthViewer(QFrame):
    """Square-ish live 2.5D pane synchronized to the same waveform stream."""
    def __init__(self,parent=None):
        super().__init__(parent); self.engine=VideoSynthEngine(); self.frame=np.zeros((240,240,3),dtype=np.uint8); self.setMinimumSize(240,180)
        self.setStyleSheet('background:#050608;border:1px solid #30343c;border-radius:6px;')
    def update_from_audio(self,data):
        self.engine.set_waveform(data); self.frame=self.engine.render_frame(max(240,self.width()),max(180,self.height())); self.update()
    def paintEvent(self,event):
        super().paintEvent(event); q=QPainter(self); h,w=self.frame.shape[:2]; qimg=QImage(self.frame.data,w,h,w*3,QImage.Format.Format_RGB888); q.drawImage(self.rect(),qimg.copy())
...
class PhaseLockedWavefieldEngine:
    ...
    def apply_phase_locked_randomization(self):
        """
        Takes user composition as a carrier wave, anchors it to seed-derived
        control points, and utilizes Euclidean distribution geometry for total phase lock.
        """
        numeric_seed = self.get_numeric_seed()
        rng = np.random.default_rng(numeric_seed)

        # Handle decimal sequence lengths gracefully via floor conversion for discrete steps
        count = int(self.app.spin_seq_length.value()) if hasattr(self.app, 'spin_seq_length') else 16

        # Euclidean distribution pulse spacing derived from seed
        pulses = max(1, int(count * (numeric_seed % 7 + 1) / 12))
        euclidean_grid = [((i * pulses) % count) < pulses for i in range(count)]

        for name in self.app.instrument_names_48:
            mem = self.app.instrument_sequencer_memory[name]

            for s in range(count):
                # 1. Carrier wave: User's existing composition state
                user_active = mem["steps"][s] if s < len(mem["steps"]) else False
                user_amp = mem["amplitudes"][s] if s < len(mem["amplitudes"]) else 0.5

                # 2. Control points: Seed-derived harmonic modulation anchor
                control_phase = (s / count) * 2 * np.pi + (numeric_seed * 0.05)
                control_envelope = 0.5 * (1.0 + np.sin(control_phase))

                # 3. Euclidean Coherence Gate
                is_euclidean_locked = euclidean_grid[s]

                # 4. Phase-locking logic (Carrier wave + seed alignment)
                if is_euclidean_locked:
                    final_step = True
                    final_amp = max(user_amp, float(control_envelope))
                else:
                    final_step = user_active and (rng.random() > 0.3)
                    final_amp = user_amp * control_envelope

                # Apply back to memory safely
                if s < len(mem["steps"]):
                    mem["steps"][s] = bool(final_step)
                    mem["amplitudes"][s] = float(np.clip(final_amp, 0.0, 1.0))

        if hasattr(self.app, 'reload_active_instrument_sequencer_ui'):
            self.app.reload_active_instrument_sequencer_ui()

        print(f"[Phase-Locked Loop] Wavefield synchronized. Seed: {self.app.input_seed_val.text()}")
    ...
    def generate_ideal_patch_bay_routing(self):
        ...
        """
        if not hasattr(self.app, 'patch_connections'):
            self.app.patch_connections = []
        else:
            self.app.patch_connections.clear()

        num_operators = len(self.app.instrument_names_48)
        numeric_seed = self.get_numeric_seed()
        rng = np.random.default_rng(numeric_seed)
        ...
        for source_idx in range(num_operators):
            ...
            if target_idx != source_idx and target_idx not in assigned_inputs:
                source_name = self.app.instrument_names_48[source_idx]
                target_name = self.app.instrument_names_48[target_idx]
                ...
                self.app.patch_connections.append(connection)
                assigned_inputs.add(target_idx)

        print(f"[Patch Bay Optimizer] Generated {len(self.app.patch_connections)} non-redundant links.")
...
class MathematiciansGrooveboxApp(QMainWindow):
    ...
    def init_ui_components(self):
        ...
        self.input_seed_val = QLineEdit()
        self.input_seed_val.setText("0.0")  # Global seed defaults to 0.0
        ...
        self.btn_seeded_randomize.clicked.connect(self.apply_seeded_harmonic_randomization)
        ...
        self.spin_tuning.setRange(0.0, 50000.0)
        self.spin_tuning.setDecimals(4)
        self.spin_tuning.setSingleStep(0.1)
        self.spin_tuning.setValue(440.0)
        ...
        self.top_layout.addWidget(QLabel("Base Frequency:"))
        ...
        sizing_layout.addWidget(QLabel("Global Playlist Rows:"))
        ...
        sizing_layout.addWidget(QLabel("Global Convolve:"))
        ...
        sizing_layout.addWidget(self.slider_global_convolve, 1)
        ...
        seq_header_layout.addWidget(QLabel("PKP NullLock = Active Instrument:"))
        seq_header_layout.addWidget(self.top_sequencer.instance_combo)

        btn_trigger_seq = QPushButton("▶ Trigger PKP NullLock")
        btn_trigger_seq.clicked.connect(lambda: print(f"[Resonance Nullifier] Geometric phase-lock active for {self.top_sequencer.instance_combo.currentText()}"))
        ...
        self.steps_inner_layout.setSpacing(4)
        ...
        self.steps_scroll.setMinimumHeight(92)
        ...
        seq_inner.addWidget(self.steps_scroll, 1)
        master_container.addWidget(self.top_sequencer, 1)

        # Live audiovisual views: square-ish, side-by-side, equal weight.
        self.visual_oscilloscope = VisualOscilloscope()
        self.video_synth_viewer = VideoSynthViewer()
        visual_split = QSplitter(Qt.Orientation.Horizontal)
        visual_split.addWidget(self.visual_oscilloscope)
        visual_split.addWidget(self.video_synth_viewer)
        visual_split.setStretchFactor(0, 1)
        visual_split.setStretchFactor(1, 1)
        visual_split.setChildrenCollapsible(False)
        visual_split.setMinimumHeight(240)
        master_container.addWidget(visual_split, 1)
    ...
    def _engine_signature(self, source):
        """Stable signature of user/program state. Global controls are intentionally excluded."""
        h=[]
        for name in self.instrument_names_48:
            mem=self.instrument_sequencer_memory.get(name,{})
            h.append((name,tuple(bool(x) for x in mem.get("steps",[])),tuple(round(float(x),5) for x in mem.get("amplitudes",[])),tuple(round(float(x),5) for x in mem.get("pitches",[]))))
        blends=tuple(round(float(r.get("blend",1.0)),6) for r in getattr(self,"master_playlist_data",[]))
        return (source, self.input_seed_val.text().strip() if hasattr(self,"input_seed_val") else "0.0", tuple(h), blends)

    def _randomize_playlist_blends(self, rng):
        for row in getattr(self,"master_playlist_data",[]) or []:
            row["blend"] = float(rng.random())

    def apply_euclidean_and_idealized_rhythms(self):
        sig=self._engine_signature("euclidean")
        if self._engine_last_signature.get("euclidean") == sig:
            print("[Euclidean Phase-Lock] No new user changes; idempotent retoggle skipped.")
            return
        count=max(1,int(self.spin_seq_length.value()) if hasattr(self,"spin_seq_length") else 16)
        seed=self.get_numeric_seed(); rng=np.random.default_rng(seed+17)
        for i,name in enumerate(self.instrument_names_48):
            mem=self.instrument_sequencer_memory[name]; self._ensure_seq_mem_length(mem,count) if hasattr(self,"_ensure_seq_mem_length") else None
            pulses=max(1,(i%7)+1)
            for s in range(count):
                if mem["steps"][s]:
                    continue  # user/programmed state is stable
                is_eu=((s*pulses)%count)<pulses
                if is_eu:
                    mem["steps"][s]=True; mem["amplitudes"][s]=max(1.0,float(mem["amplitudes"][s])); mem["probabilities"][s]=100
        self._randomize_playlist_blends(rng)
        self._engine_last_signature["euclidean"]=self._engine_signature("euclidean")
        self.reload_active_instrument_sequencer_ui()
        print("[Euclidean Phase-Lock] Applied once; future retoggles are no-ops until user state changes.")
    ...
    def apply_seeded_harmonic_randomization(self):
        sig=self._engine_signature("randomizer")
        if self._engine_last_signature.get("randomizer") == sig:
            print("[Randomizer] No new user changes; idempotent retoggle skipped.")
            return
        numeric_seed=self.get_numeric_seed(); rng=np.random.default_rng(numeric_seed)
        count=max(1,int(self.spin_seq_length.value()) if hasattr(self,"spin_seq_length") else 16)
        for i,name in enumerate(self.instrument_names_48):
            mem=self.instrument_sequencer_memory[name]
            if hasattr(self,"_ensure_seq_mem_length"): self._ensure_seq_mem_length(mem,count)
            for s in range(count):
                if mem["steps"][s]:
                    continue
                if rng.random()<0.40:
                    ...
                    mem["probabilities"][s]=int(rng.integers(70,101))
            # Do not overwrite user scripts.
            existing=self.instrument_scripts.get(name,"")
            if not existing or existing.startswith("# Script workspace"):
                m=float((i%11)+1)*(MEUM_CONSTANT/1.5)
                self.instrument_scripts[name]=(f"# Seeded Geometric Resonance Script [{self.input_seed_val.text()}] for {name}\n"
                    "def evaluate_wave(x, y, z):\n"
                    f"    m = {m}\n"
                    "    return np.sin(x*m)*np.cos(y/m)-np.tanh(z*0.5)")
        self._randomize_playlist_blends(rng)
        self._engine_last_signature["randomizer"]=self._engine_signature("randomizer")
        self.reload_active_instrument_sequencer_ui()
        print("[Seeded Randomizer] Applied once; global controls remain untouched and retoggles are idempotent.")
    def generate_ideal_patch_bay_routing(self):
        """
        Builds a non-redundant, acyclic routing network across operators
        using the current irrational seed as a deterministic path weight metric.
    ...
    def _push_live_audiovisual_wave(self, data):
        """Single direct data stream to both visual panes; PKP NullLock remains a global modulation."""
        if isinstance(getattr(self,"visual_oscilloscope",None), VisualOscilloscope):
            self.visual_oscilloscope.update_waveform(np.asarray(data))
        if hasattr(self,"video_synth_viewer"):
            self.video_synth_viewer.update_from_audio(np.asarray(data))

    def apply_global_convolve_waveforms(self, waveforms, strength=None):
        """Normalized structural spectral convolution; user-facing synth parameters are excluded."""
        arrays=[np.asarray(w,dtype=float).ravel() for w in (waveforms or []) if np.asarray(w).size]
        if not arrays: return None
        n=min(map(len,arrays)); arrays=[a[:n] for a in arrays]
        base=np.mean(np.stack(arrays),axis=0); base/=max(np.max(np.abs(base)),1e-12)
        if strength is None: strength=(self.slider_global_convolve.value()/100.0) if hasattr(self,"slider_global_convolve") else 0.0
        if strength<=0: return base
        spec=np.fft.rfft(arrays[0])
        for a in arrays[1:]:
            spec*=np.fft.rfft(a); spec/=max(np.max(np.abs(spec)),1e-12)
        conv=np.fft.irfft(spec,n=n); conv/=max(np.max(np.abs(conv)),1e-12)
        out=(1-strength)*base+strength*conv; out/=max(np.max(np.abs(out)),1e-12)
        return out
    ...
    def toggle_playback(self):
        """Start/stop the live audiovisual transport without treating PKP NullLock as a timeline event."""
        if getattr(self,"_live_audio_playing",False):
            self._live_audio_playing=False
            if HAS_SOUNDDEVICE if 'HAS_SOUNDDEVICE' in globals() else False:
                try: sd.stop()
                except Exception: pass
            self.btn_play.setText("▶ PLAY Audiovisual Track")
            print("[System] Audiovisual Track paused/stopped.")
            return
        self._live_audio_playing=True
        self.btn_play.setText("⏸ PAUSE Audiovisual Track")
        # The existing renderer/export path can be connected here by the later live engine.
        print("[System] Audiovisual Track live stream armed. PKP NullLock remains global and is injected at trigger time.")

    def stop_playback(self):
        self._live_audio_playing=False
        try:
            if HAS_SOUNDDEVICE: sd.stop()
        except Exception: pass
        if hasattr(self,"btn_play"): self.btn_play.setText("▶ PLAY Audiovisual Track")
        print("[System] Live audiovisual playback stopped.")
