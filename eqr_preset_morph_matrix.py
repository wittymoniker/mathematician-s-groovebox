# Filename: eqr_preset_morph_matrix.py
...
class PresetMorphMatrixPage(QWidget):
    """
    EQR 34-Constant Preset Morph & Interpolation Matrix featuring skeuomorphic
    rotary controllers for real-time morphing between structural harmonic states.
    """
    def __init__(self, engine):
        ...
        layout = QVBoxLayout()
        group = QGroupBox("EQR 34-Constant Real-Time Preset Morph Matrix")
        grid = QGridLayout()

        morph_channels = [
            ("Morph Index Alpha", 0.0, 1.0, 0.0),
            ("Morph Index Beta", 0.0, 1.0, 0.33),
            ("Morph Index Gamma", 0.0, 1.0, 0.66),
            ("Morph Index Delta", 0.0, 1.0, 1.0),
            ("Plastic Constant Blend", 1.0, 2.0, EQR_CONSTANTS["PLASTIC_NUMBER"]),
            ("Silver Ratio Crossfade", 1.0, 3.0, EQR_CONSTANTS["SILVER_RATIO"]),
            ("Supergolden Interpolator", 1.0, 2.0, EQR_CONSTANTS["SUPERGOLDEN"]),
            ("MEUM Morph Weight", 0.0, 2.0, MEUM)
        ]

        for idx, (label, min_v, max_v, def_v) in enumerate(morph_channels):
            r = idx // 4
            c = idx % 4
